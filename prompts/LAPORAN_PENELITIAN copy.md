...
Fitur ini mengelola persistensi data terstruktur, seperti data pengguna dan riwayat percakapan. Menggunakan **SQLAlchemy ORM**, fitur ini mengabstraksi interaksi dengan database relasional (misalnya, PostgreSQL), menyediakan fungsi untuk operasi CRUD (Create, Read, Update, Delete) pada tabel `users` dan `chat_history`.
...
Sistem ini memanfaatkan database relasional untuk menyimpan data terstruktur seperti informasi pengguna dan riwayat interaksi. Terdapat relasi *one-to-many* antara tabel `users` dan `chat_history`.

**Tabel Pengguna (`users`)**
Tabel ini menyimpan informasi kredensial dan data dasar setiap pengguna yang terdaftar.
| Nama Kolom | Tipe Data | Deskripsi |
| :--- | :--- | :--- |
| `id` | `INTEGER` | Primary Key, sebuah nomor identifikasi unik untuk setiap pengguna. |
| `username` | `VARCHAR` | Nama pengguna yang akan ditampilkan. |
| `email` | `VARCHAR` | Alamat email pengguna, digunakan untuk login dan harus unik. |
| `password_hash` | `VARCHAR` | Hash dari kata sandi pengguna, disimpan untuk keamanan. |
| `created_at` | `TIMESTAMP` | Cap waktu yang menandakan kapan akun pengguna dibuat. |

**Tabel Riwayat Percakapan (`chat_history`)**
Tabel ini mencatat setiap pesan yang dikirim oleh pengguna dan diterima dari chatbot, memungkinkan percakapan untuk ditampilkan kembali.
| Nama Kolom | Tipe Data | Deskripsi |
| :--- | :--- | :--- |
| `id` | `INTEGER` | Primary Key, ID unik untuk setiap pesan. |
| `user_id` | `INTEGER` | Foreign Key yang mereferensikan `users.id`, menghubungkan pesan ke pengguna tertentu. |
| `role` | `VARCHAR` | Peran pengirim pesan, yaitu 'user' untuk pengguna atau 'assistant' untuk chatbot. |
| `content` | `TEXT` | Isi teks dari pesan yang dikirim atau diterima. |
| `timestamp` | `TIMESTAMP` | Cap waktu yang menandakan kapan pesan dikirim. |
...
*Gambar 3.5: Halaman Utama Chatbot*
...
*Gambar 3.6: Halaman Unggah Materi*
...
##### 3.4.3.1 Penjelasan Kode Frontend
*   **`main.py`**: Merupakan file utama yang menjalankan antarmuka pengguna (frontend). Dibangun menggunakan **Streamlit**, sebuah framework Python yang memungkinkan pembuatan aplikasi web interaktif dengan cepat. File ini bertanggung jawab untuk merender semua komponen UI, menangani input dari pengguna (seperti pertanyaan dan login), menampilkan riwayat percakapan, dan berkomunikasi dengan backend melalui permintaan API.
*   **`static/style.css`**: File ini berisi aturan CSS kustom untuk memberikan tampilan dan nuansa yang unik pada aplikasi Streamlit, memodifikasi gaya default agar sesuai dengan identitas visual proyek.
...
1.  **Azure Container Registry (ACR):** Layanan ini digunakan sebagai registri Docker privat untuk menyimpan *image* `frontend` dan `backend` yang telah dibangun.
    ...
    *Gambar 3.7: Azure Container Registry*
2.  **Build dan Push Image:** Setelah *image* Docker berhasil dibangun secara lokal, *image* tersebut diunggah (di-*push*) ke Azure Container Registry.
    ...
    *Gambar 3.8: Proses Push Image ke ACR*
3.  **Azure App Service:** Dua instansi App Service—layanan PaaS (Platform-as-a-Service) dari Azure—dibuat. Satu untuk menjalankan kontainer `frontend` dan satu lagi untuk `backend`. App Service dikonfigurasi untuk menarik *image* yang sesuai dari ACR dan menjalankannya.
    ...
    *Gambar 3.9: Konfigurasi App Service*
4.  **Konfigurasi Variabel Lingkungan:** Informasi sensitif seperti kunci API Google (`GOOGLE_API_KEY`) tidak disimpan dalam kode, melainkan dikonfigurasi sebagai variabel lingkungan di pengaturan aplikasi App Service untuk keamanan.
    ...
    *Gambar 3.10: Pengaturan Variabel Lingkungan*
